...
const TNode* objPosBST::findMin(const TNode* thisNode) const
{
    ...
    if(thisNode == nullptr){
        ...
        return findMin(thisNode->left); //recursion strucutre 
    }
...
void objPosBST::remove(const objPos &thisPos, TNode* &thisNode)
{
    ...
    if(thisNode == NULL){
        ...
        char Prefix = findMin(thisNode->right)->data.getPF();
        thisNode->data.setPF(Prefix); //absorb the old data mouted
...
bool objPosBST::findGreater(const int numThreshold, const TNode* thisNode) const
{
    ...
    if(thisNode == NULL){
        ...
        return findGreater(numThreshold,thisNode->right);
    }
    else{
        return true;
    }
