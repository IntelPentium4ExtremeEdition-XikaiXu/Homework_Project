...
Matrix::Matrix() //inti the data of the matirx just call it.
{
	// Default Constructor
	...
	for(int i = 0; i < rowsNum; i++) 
		for(int j = 0; j < colsNum; j++)
			matrixData[i][j] = 0; //init the matrix data == 0;
	//init the matrix 
}
...
{	
	...
	if(row <= 0){ // make  the correct defualt situation.
		rowsNum = 3;
	}
	else{
		rowsNum = row;
	}
	if(col <= 0){ // can be 2d if col = 0;;;;;
		colsNum = 3;
	}
	else{
		colsNum = col;
	}
	 // first create the colns as vertical with [0] x extension
	matrixData = new int*[rowsNum]; // make the neo row setup
	// note: creat a 2d demision, second function repersent each branch have the mount of space.
	for (int i = 0; i < rowsNum; i++){  //for memory, using the intergrated stuff
		matrixData[i] = new int[colsNum]; // make new space for col 
	}
...
Matrix::~Matrix(){
	for(int i = 0; i < rowsNum; i++){
		delete[] matrixData[i]; //delete each colns of element from each row position!!!
	}
	delete[] matrixData;
}

Matrix::Matrix(int row, int col, int** table) //pass by reference 
{
	...
	// Additional Constructor
	for(int a = 0; a < rowsNum; a++){ //copying process, directed operated on the table 
		for(int b = 0; b < colsNum; b++){
			matrixData[a][b] = table[a][b];
		}
	}
}
...
int Matrix::setsizeofrows(){
	rowsNum = sizeof(matrixData) / sizeof(matrixData[0]);
}

int Matrix::getsizeofcols(){
	// Getter - Return the column size of this matrix
	return colsNum;
	// IMPLEMENT YOUR CODE BELO
}

int Matrix::setsizeofcolns(){
	colsNum = sizeof(matrixData[0])/sizeof(matrixData[0][0]);
}

bool Matrix::setElement(int x, int i, int j){

	// Getter Method - Get the value of the element located at index [i, j]
	if(j >= getsizeofcols() || i >= getsizeofrows()){
		return false;
	}
	else if(j < 0 || i < 0){
		return false;
		...
		matrixData[i][j] = x;
		return true;
	}// we need to using the ext formation.
...
Matrix Matrix::copy(){
	...
	Matrix copy = Matrix(rowsNum,colsNum); //nope,this is c++'s shalow copy

	for(int i = 0; i < rowsNum; i++) //wrong dude. mess up x and y
		for(int j = 0; j < colsNum; j++)
	...
	/* UNCOMMENT THIS SECTION, THEN DEBUG! */
					...
					//change it to pass by reference lol
void Matrix::addTo( Matrix &m ){ //similar to the pointer method, will not acceledentlly del the heap
	...
	// IMPLEMENT YOUR CODE BELOW
	//check the rows and cols legth
	//directed the matrix m's characteristic!!!!
	if(m.rowsNum != rowsNum || m.colsNum != colsNum){
		...
		for(int y = 0; y < rowsNum; y++){
			for(int x = 0; x < colsNum; x++){
				matrixData[y][x] += m.matrixData[y][x];
			}						//m is a specific class
		}
	}
}
...
Matrix Matrix::subMatrix(int i, int j){
	...
	// IMPLEMENT YOUR CODE BELOW
	//if we want to return a matrix, we can directly make a class that contains all thing we needed
	Matrix temp(i+1,j+1);//creat the correct size temp
	if(i >= rowsNum || j >= colsNum){
		...
		for(int y = 0; y < i+1; y++){
			for(int x = 0; x < j+1; x++){
				temp.matrixData[y][x] = matrixData[y][x];
			}
		}
	}
	return temp;
