...
// GLOBAL VARIABLES
// ================================
int exitFlag;   // Program Exiting Flag, used to determine whether to leave the program loop and shutdown the program//GET INFO FROM INPUT
char displayString[20]={'>','>'},cmd; //INITIAL LIST ENVIRONMENT WITH 2 >> CAPITAL
int start_pos;                     //SET ROLLING POSITION
int i,add;    //JUST A PRINT INDEX CYCLE STUFF
int back_test; //signal of reverse
//THE MOUNT OF INPUT
...
int main(void)
{
    // Start-Up
    Initialize();  
    for(i = 2;i<=19;i++){          //fullfilled the list with ' 'as the bit token 
        displayString[i] = ' ';
    }
...
void Initialize(void)
{
    ...
    start_pos = 0; // start pos means where the text needs to print
    add = 2;       // from list[2] put char inside 
...
void GetInput(void)
{   // reason for large getinput function
    //want to have less operation to the list,my Lubuntu terminal have some problem once have too much call list, will automaticlly stucked
    displayString[add] = '<';     //the end of the list
    displayString[add+1] = '<';
    if(MacUILib_hasChar()&&add<18){   //max input 16 characters
        cmd = MacUILib_getChar();
        if (cmd > 32 && cmd<127){
            displayString[add] = cmd;
            add++;
            displayString[add+1] = '<';  //once the old location has been covered by the data, new< interact
        }
    }
    else if(MacUILib_hasChar()&&add>=18){    //only used for ascii function detection.
        cmd = MacUILib_getChar();
    }
    if (cmd == 27){                       //exit when esc push
        exitFlag = 1;
    } 
    if (cmd == 32){                       //inverse when space push 
        back_test = 1;
    }
...
{   
    ...
    if (back_test){              //if back_test = 1, inverse 
        start_pos--;
        if(start_pos < 0){;
            start_pos = 20;
        }
...
{
    MacUILib_clearScreen();
    MacUILib_printf("%s\n", "McMaster Marquee Display");
    MacUILib_printf("%s\n", "====================");
    for(i = 0; i<=19;i++){
        printf("%c", displayString[(start_pos + i)%20]);    //from startpos to the start of the list
